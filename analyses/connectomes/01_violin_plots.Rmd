...
R_stream_aov <-
  R_data_stream[,right_rois] %>%
  unique() %>%
  melt() 

aov_mdl <- aov(value ~ variable, data = R_stream_aov)
aov_mdl %>%
  summary()
...
L_stream_aov <-
  L_data_stream[,left_rois] %>%
  unique() %>%
  melt() 

aov_mdl <- aov(value ~ variable, data = L_stream_aov)
aov_mdl %>%
  summary()
...
# Volin Plots - Separate plots ( NOT USED for the paper)
```{r}
# LEFT
left_rois <- c(#"L_ERC/L_CA4DG",
               "L_CA2CA3/L_CA4DG",
...
# Left and Right in pairs (NOT USED for the paper)
```{r}
dotsize <- c(1, 0.0001, 0.0001, 0.0001, 0.0001)
binwidth <- c(1, 0.0001, 0.0001, 0.0001, 0.0001)
...
stream_sex <- stream_sex %>%
  ...
  mutate("CA2CA3/CA1" = (`R_CA2CA3/R_CA1`+`L_CA2CA3/L_CA1`)/2) %>%
...
(ggplot(temp_heat_clean, aes(x=variable, y=value, fill=variable)) +
  ...
  ggsave(paste0("../../output/plots/violin/", "sex_differences", "_",
                "all_rois", ".png"), .,
         width = 8, height = 3 #,
         #bg = "transparent"
         )
...
for (i in 1:6){
  print(sex_rois[i])
  t.test(stream_sexM[,sex_rois[i]], stream_sexF[,sex_rois[i]]) %>%
    print()
}
